...
<div class="main-panel">
    <div class="content-wrapper">
        ...
        <a href="javascript:void(0)" class="btn btn-sm btn-primary addBtn"><i class="las la-plus"></i>@lang('Add New')</a>
        @else
                                    ...
                                    <div class="thumb">
                                        ...
                                        <div class="avatar-edit">
                                            <input type="file" class="profilePicUpload" name="image_input[{{ $imgKey }}]"
                                                id="addImage{{ $loop->index }}" accept=".png, .jpg, .jpeg">
                                            <label for="addImage{{ $loop->index }}" class="bg--primary">{{
                                                __(keyToTitle($imgKey)) }}</label>
                                            <small class="mt-2  ">
                                                @if(@$section->element->images->$imgKey->size)
                                                @lang('Recomended size:'):
                                                <b>{{@$section->element->images->$imgKey->size}}</b> @lang('px').
                                                @endif
                                            </small>
                                        </div>
                                    </div>
        ...
        <div id="updateBtn" class="modal fade" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <form action="{{ route('admin.frontend.sections.content', $key) }}" class="edit-route" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="type" value="element">
                            ...
                            <div class="form-group">
                                ...
                                <div class="image-upload">
                                    <div class="thumb">
                                        ...
                                        <div class="avatar-edit">
                                            <input type="file" class="profilePicUpload" name="image_input[{{ $imgKey }}]"
                                                id="fileUploader{{ $loop->index }}" accept=".png, .jpg, .jpeg">
                                            <label for="fileUploader{{ $loop->index }}" class="bg--primary">{{
                                                __(keyToTitle($imgKey)) }}</label>
                                            <small class="mt-2  ">
                                                @if(@$section->element->images->$imgKey->size)
                                                @lang('Recomended size:'):
